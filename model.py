...
class ActorNetwork(nn.Module):
    ...
    def forward(self, x):
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        # Softmax ensures we get a probability distribution over the 12 phases
...
class CriticNetwork(nn.Module):
    ...
    def forward(self, x):
        # 'x' is the concatenated state of all agents (Centralized Training)
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
...
class MAPPOAgent:
    ...
    def select_action(self, obs):
        ...
        # Ensure we are tracking gradients for the log_prob
        probs = self.actor(state)
        dist = torch.distributions.Categorical(probs)
        action = dist.sample()
        
        # We return the log_prob itself to keep the gradient link
